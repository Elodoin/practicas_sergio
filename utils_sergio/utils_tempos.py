...
def tempos(H, h, D, v=np.array([4.2, 6.3, 7.5, 8, 8, 8]), z=np.array([0, 4, 12, 18, 20, 32])):
    ...
    H = np.around(H, 10)
    D = np.around(D, 10)
    # Alturas y velocidades de capas en Canarias (en km y km/s)
    # Trabajaremos con la velocidad de la onda p por defecto,
    # pudiendo hallar el tiempo de la s multiplicando la salida por 1.78.
    ...
    if h < 0:
        velocidad = v[0]
    # En el array tempos guardaremos los tiempos de llegada de cada rayo
    # y en el array capa la capa en que se produce la refraccion critica,
    # asignandole el valor de 0 al rayo directo.
    tempos = []
    pes = []
    capa = []

    # Distinguimos ahora los casos en que el terremoto se produce a una
    # altura superior a la del sismometro y viceversa.
    # Comenzamos con el caso en que el terremoto se produce por debajo del
    # sismometro:
    if h + H >= 0:
        # Situamos el origen de las capas en el sismometro, de forma que
        #  variamos la profundidad de las capas y llamamos h a la profundidad
        #  del terremoto para este nuevo sistema de referencia.
        z = np.concatenate(([0], z[1:] + H))

        # La siguiente linea de codigo nos solventa el problema que surge cuando
        # el sismometro se produce a una profundidad > a z[1].
        z1 = z[z >= 0]
        ...
        # Para estudiar el rayo directo, a las que estan por encima le anadimos
        # la profundidad del evento y calculamos las distancias entre capas
        direc1 = np.concatenate((direc, [h]))
        zp = direc1[1:]-direc1[:-1]

        if h > 0:
            # Introducimos una funcion que nos permite hallar el parametro p 
            # (equivalente a hallar el angulo de incidencia) para el rayo directo.
            def func(p):
                u = D - np.sum(zp/np.sqrt((v[:len(zp)]*p)**(-2)-1))
                return u

            # Buscamos el cero de esta funcion mediante el metodo numerico brentq.
            # (Ojo!, el metodo converge bien pero en ciertos casos advierte de una 
            # division por cero que puede ralentizar el programa)
        ...
        for n in range(1, len(refrac)+1):
            # En direc2 guardamos las capas que atraviesa el rayo cuando sube desde la
            # refraccion critica hasta el sismometro (trayectoria de subida); y en refrac2
            #  a las capas que atraviesa al bajar desde su hipocentro hasta la refraccion
            # critica (trayectoria de bajada). Ademas, a la trayectoria de bajada le anadimos
            # la profundidad del terremoto. Hallamos a continuacion las distancias entre capas
            ...
            # Se puede comprobar que para estos rayos que sufren refraccion critica el valor
            # del parametro p viene dado por p = 1/vn, siendo vn la velocidad de propagacion
            # en la capa en que se produce la refraccion critica.
            p = 1/v[len(direc2)-1]

            # Introducimos una funcion que nos permite hallar la distancia dn que recorre el
            # rayo por la capa de refraccion critica.
            def fanc(dn):
                u = D - np.sum(zp/np.sqrt((v[:len(zp)]*p)**(-2)-1)) - np.sum(
                    zr/np.sqrt((v[len(direc)-1: n+len(direc)-1]*p)**(-2)-1)) - dn
                return u

            # Descartamos aquellas refracciones que no pueden darse pues se supera el angulo
            # critico: fanc(0)<= 0:
            if fanc(0) > 0:
                # Buscamos numericamente el valor de dn para cada rayo y hallamos el tiempo
                # empleado, donde tenemos en cuanto las tres trayectorias distinguibles seguidas
                # por el rayo: bajada, critica y subida, guardandolo en la lista tempo asi como la capa critica que alcanzo.
                dn = brentq(fanc, 0, D)
                t = np.sum(zp/(v[:len(zp)]*np.sqrt(1-(p*v[:len(zp)])**2))) + np.sum(zr/(
                    v[len(direc)-1: n+len(direc)-1]*np.sqrt(1-(p*v[len(direc)-1: n+len(direc)-1])**2))) + dn*p
                tempos.append(t)
